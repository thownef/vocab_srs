                                ...
                                <div class="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow"
                                    ...
                                    <div class="flex justify-end gap-2">
                                        <button wire:click="markForgotten({{ $review['vocabulary_word_id'] }})"
                                            ...
                                            <span wire:loading.remove wire:target="markForgotten({{ $review['vocabulary_word_id'] }})">
                                                <i class="fas fa-times mr-2"></i>
                                                Quên
                                            </span>
                                            <span wire:loading wire:target="markForgotten({{ $review['vocabulary_word_id'] }})">
                                        ...
                                        <button wire:click="markReviewed({{ $review['vocabulary_word_id'] }})"
                                            ...
                                            wire:loading.attr="disabled">
                                            <span wire:loading.remove wire:target="markReviewed({{ $review['vocabulary_word_id'] }})">
                                                <i class="fas fa-check mr-2"></i>
                                                Đã nhớ
                                            </span>
                                            <span wire:loading wire:target="markReviewed({{ $review['vocabulary_word_id'] }})">
                                                <i class="fas fa-spinner fa-spin mr-2"></i>
                                                Đang xử lý...
                                            </span>
                                        </button>
